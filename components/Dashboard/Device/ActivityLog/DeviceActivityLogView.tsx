  ...
  const fetchLogs = useCallback(async (page: number, limit: number) => {
    ...
    try {
      ...
      if (response.status === 200) {
        setActivityLogs(prevLogs => [...prevLogs, ...response.data.results]);
        ...
        <div key={index}>
          ...
          <div className='p-3 mb-3 rounded-lg'>
            <div className='flex flex-row items-start md:items-center gap-1'>
              <div>
                {item.user ? <div className={`w-10 h-10 rounded-full mr-5 flex justify-center items-center`}>
                  <Image
                    src={item.user.profile ? item.user.profile : '/dummyAvatar.png'}
                    alt="User avatar"
              ...
              <div className=' flex flex-col'>
                ...
                <div className='flex flex-col md:flex-row md:items-center gap-1'>
                  {item.user && <p className='!mb-0'>{item.user.firstName + ' ' + item.user.lastName}</p>}
