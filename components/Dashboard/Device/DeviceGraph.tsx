...
const DeviceGraph = ({ id }: DeviceGraphProps) => {
  ...
  const [currentActivePreset, setCurrentActivePreset] = useState('Today')
    ...
    if (deviceOem) {
      try {
        ...
        if (response.status === 200) {
          ...
          const temperatureData = sortedData.map((point: DataPoint) => ({
            date: new Date(point.createdAt),
            value: point.temperature,
          }));

          const humidityData = sortedData.map((point: DataPoint) => ({
            date: new Date(point.createdAt),
            value: point.relativeHumidity,
          }));
          setData(sortedData);
  ...
  const handleDatePreset = (preset: string) => {
    ...
    switch (preset) {
      ...
      case 'Last 30 Days':
        ...
        break;
      default:
        newRange = [dayjs().subtract(1, 'day').startOf('day'), dayjs().endOf('day')];
    }
  ...
  const menu = (
    <Menu>
      <Menu.Item onClick={() => handleDatePreset('Today')}>Today</Menu.Item>
      <Menu.Item onClick={() => handleDatePreset('Yesterday')}>Yesterday</Menu.Item>
      <Divider className='!m-0 !h-[1px] bg-[#141414]' />
      <Menu.Item onClick={() => handleDatePreset('This Week')}>This Week</Menu.Item>
      <Menu.Item onClick={() => handleDatePreset('Last Week')}>Last Week</Menu.Item>
      <Divider className='!m-0 !h-[1px] bg-[#141414]' />
      <Menu.Item onClick={() => handleDatePreset('Last 3 Days')}>Last 3 Days</Menu.Item>
      <Menu.Item onClick={() => handleDatePreset('Last 7 Days')}>Last 7 Days</Menu.Item>
      <Menu.Item onClick={() => handleDatePreset('Last 30 Days')}>Last 30 Days</Menu.Item>
    </Menu>
  );
    ...
    series: [
      {
        ...
        tooltip: {
          renderer: ({ datum, xKey, yKey }) => {
            ...
            const dateFormatted = new Date(xValue).toLocaleDateString("en-GB", {
              day: "2-digit",
              month: "short",
              year: "numeric",
            });
    ...
    const [chartOptions, setChartOptions] = useState<AgChartOptions>({
      ...
      axes: [
        ...
        },
        {
          ...
          position: "left",
          label: {
            formatter: (params) => `${params.value}°C`, // Adding °C for degree Celsius
          },
        },
      ],
    });

    return <AgChartsReact options={chartOptions as any} />;
  };
    ...
    const [chartOptions, setChartOptions] = useState<AgChartOptions>({
      ...
      axes: [
        ...
        },
        {
          ...
          position: "left",
          label: {
            formatter: (params) => `${params.value}%`, // Adding °C for degree Celsius
          },
        },
      ],
    });
