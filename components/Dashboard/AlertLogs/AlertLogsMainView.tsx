    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setAlertLogs((prevState) =>
          prevState.map((item) => (item.id === id ? { ...item, ...responseNote, accepted: true } : item))
        );
      }
    } catch (error) {
      console.log(error);
  ...
  const columns: TableProps<any>["columns"] = [
    {
      title: "ALERT NAME",
      dataIndex: "name",
      render: (_, { alert }) => <p className="!text-black">{alert?.name}</p>,
    },
    {
      title: "ALERT REASON",
      dataIndex: "type",
      ...
      render: (_, { createdAt }) => {
        ...
        return (
          <p className="!text-black">
            {formatedDateTime.formattedDate} {formatedDateTime.formattedTime}
          </p>
        );
      },
    },
    ...
    },
    {
      ...
      render: (_, { id, notes }) => {
        return (
          <Button type="link" onClick={() => showModal(id, notes || "")}>
            Show & Edit Note
          </Button>
        );
      },
      ...
      render: (_, { id, accepted }) => {
        return (
          <Checkbox
            ...
            onChange={(e) => handleAlertLogAccepted(e, id, accepted)}
            disabled={accepted}
            checked={accepted}
          />
        );
    ...
    <Card>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10">
        <div className=" flex flex-row w-full flex-wrap md:w-auto justify-between items-center !mb-4 md:!mb-0">
          <h1 className="text-2xl sm:text-3xl font-semibold !mb-0">Alert Logs</h1>
        </div>
        <div className=" flex flex-col md:flex-row items-end gap-4 w-full md:w-auto">
          <div className=" w-full md:w-auto order-2 md:order-1">
            <p className=" text-sm mb-1">Date Range:</p>
            <RangePicker onChange={handleRangeChange} defaultValue={range} className="w-full" />
          </div>
        </div>
      </div>
