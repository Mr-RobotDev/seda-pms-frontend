...
import { getDashboards, setCurrentDashboard } from '@/app/store/slice/dashboardSlice'
...
const ReportsMainView = () => {
  ...
  const { dashboards, currentDashboard } = useSelector((state: RootState) => state.dashboardReducer)
  ...
  return (
    <div>
      <>
        {dashboards.length !== 0 && (
          <div className=' flex items-center gap-2 justify-between'>
            <div className=' flex items-center gap-2'>
              <DashboardMenu dashboardsList={dashboards} routingFunctionality={false} />
            </div>
            {
              !createNewReport && 
              <div className="flex justify-center" onClick={() => setCreateNewReport(true)}>
              <span
        ...
        {currentDashboard &&
          <ReportsTable currentDashboard={currentDashboard} createNewReport={createNewReport} setCreateNewReport={setCreateNewReport} />
        }
      </>
