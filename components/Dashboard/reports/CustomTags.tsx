...
const CustomTags = ({ recipientEamils, type, setFormData }: CustomTagsProps) => {
    ...
    <>
      <div style={{ marginBottom: 16 }}>
        <div className=' flex flex-row gap-2 border-b border-b-blue-300 pb-2'>
          {
            emails.map(email => (
              <span key={email} className=' inline-block px-2 py-1 border border-gray-300 rounded-2xl'>
                <span className=' flex flex-row gap-2'>
                  {email}
                  <XCircleIcon width={20} className=' cursor-pointer' onClick={() => handleClose(email)} />
                </span>
              </span>
            ))
          }
            ...
            {inputVisible ? (
              type === 'email' ?
                <Input
                  ...
                  placeholder={type === 'email' ? 'example@example.com' : '00:00'}
                />
                :
                <div className=' flex items-center gap-2'>
                  <Select onChange={value => setSelectedTime(value)} defaultValue="00:00" style={{ width: 120 }}>
                    {options}
                  </Select>
                  <button onClick={handleInputConfirm} className=' px-3 py-1 rounded-md bg-blue-600 hover:bg-blue-700 text-white'>Add</button>
                </div>

            ) : (
              <Tag onClick={showInput} className=' cursor-pointer' style={tagPlusStyle}>
                <PlusOutlined /> {type === 'email' ? 'Add New Recipient Email' : 'Add Time'}
              </Tag>
            )}
          </div>
        </div>
